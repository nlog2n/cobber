...
using Cobber.Core.Poly;
using Cobber.Core.Poly.Visitors;

namespace Cobber.Core.Obfuscations
{
    /// <summary>
    /// ConstantEncryptObfuscation, encrypt strings as well as numerical constants.
    /// </summary>
    public class ConstantEncryptObfuscation : IObfuscation
    {
        class Phase1 : StructurePhase
        {
            public Phase1(ConstantEncryptObfuscation cc) { this.cc = cc; }
            ConstantEncryptObfuscation cc;
            public override IObfuscation Obfuscator { get { return cc; } }

            public override int PhaseID { get { return 1; } }
            public override Priority Priority { get { return Priority.CodeLevel; } }
            ...
            {
                Database.AddEntry("Const", "Type", parameters["type"] ?? "normal");
                if (parameters["type"] != "dynamic" &&
                    parameters["type"] != "native")
                {
                    ProcessSafe(target, parameters); return;
                }
                _Context txt = cc.txts[mod];
                txt.isNative = parameters["type"] == "native";
                txt.isDyn = parameters["type"] == "dynamic";
                ...
                AddHelper(modType, constTbl, HelperAttribute.NoInjection);
                ...
                AddHelper(modType, constBuffer, HelperAttribute.NoInjection);
                ...
                txt.consters = CreateConsters(txt, cobber.Random, "Initialize", constTbl, constBuffer);
            }

            private void ProcessSafe(object target, NameValueCollection parameters)
            {
                _Context txt = cc.txts[mod];
                                     ...
                                     FieldDefinition constTbl, FieldDefinition constBuffer)
            {
                ...
                txt.resKey = (rand.Next(0x20, 0x80) << 24) | (rand.Next(0x20, 0x80) << 32) |
                    (rand.Next(0x20, 0x80) << 16) | (rand.Next(0x20, 0x80) << 0);  // fixed random string encoding
                ...
                for (int i = 0; i < typeDefCount; i++)
                {
                    ...
                    AddHelper(mod, typeDef, HelperAttribute.NoInjection); // added by fanghui
                    ...
                    for (int j = 0; j < methodCount; j++)
                    {
                        ...
                        mtd.Name = Namer.GetRandomName();
                        mtd.IsCompilerControlled = true;

                        AddHelper(typeDef, mtd, HelperAttribute.NoInjection);
        ...
        {
            public Phase3(ConstantEncryptObfuscation cc) { this.cc = cc; }
            ConstantEncryptObfuscation cc;
            public override IObfuscation Obfuscator { get { return cc; } }

            public override int PhaseID { get { return 3; } }
            ...
            // extract constant information from all methods' instructions
            void ExtractData(IList<Tuple<IAnnotationProvider, NameValueCollection>> mtds,
                List<Context> txts, bool numeric, _Context txt)
            {
                foreach (var tuple in mtds)
                {
                    MethodDefinition mtd = tuple.Item1 as MethodDefinition;
                    if (cc.txts[mod].consters.Any(_ => _.conster == mtd) || !mtd.HasBody)
                        continue;
                    ...
                    for (int i = 0; i < insts.Count; i++)
                    {
                        if (insts[i].OpCode.Code == Code.Ldstr ||
                            (numeric && (
                                      insts[i].OpCode.Code == Code.Ldc_I4 ||
            ...
            public override void Process(object target, NameValueCollection parameters)
            {
                ...
                ExtractData(
                    target as IList<Tuple<IAnnotationProvider, NameValueCollection>>, txts,
                    Array.IndexOf(parameters.AllKeys, "numeric") != -1, txt);
            ...
            void FinalizeBodies(List<Context> txts)
            {
                ...
                for (int i = 0; i < txts.Count; i++)
                {
                    ...
                    int idx = txts[i].mtd.Body.Instructions.IndexOf(now);
        ...
        {
            public MdPhase1(ConstantEncryptObfuscation cc) { this.cc = cc; }
            ConstantEncryptObfuscation cc;
            public override IObfuscation Obfuscator { get { return cc; } }
        ...
        {
            public MdPhase2(ConstantEncryptObfuscation cc) { this.cc = cc; }
            ConstantEncryptObfuscation cc;
            public override IObfuscation Obfuscator { get { return cc; } }
        ...
        public string ID { get { return "const encrypt"; } }
        public string Name { get { return "Constant Encryption"; } }
        public string Description { get { return "obfuscate the constants in the code and store them in a encrypted and compressed form."; } }
        ...
        public Dictionary<string, List<string>> Parameters
        {
            get
            {
                ...
                {
                    {"numeric", new List<string> {"false","true"}},
                    {"type", new List<string> {"normal","native","dynamic"}}
                };
            }
        }
