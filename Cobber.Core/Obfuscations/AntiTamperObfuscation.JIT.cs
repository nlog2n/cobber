...
{
    partial class AntiTamperObfuscation
    {
        class JIT : IAntiTamper
        {
            ...
            public Action<object, IMemberDefinition, HelperAttribute> AddHelper { get; set; }
            public Cobber cobber { get; set; }

            public void InitPhase1(ModuleDefinition mod)
            {
                byte[] dat = new byte[27];
                cobber.Random.NextBytes(dat);
                ...
                for (int i = 1; i <= 5; i++)
                {
                    int idx = cobber.Random.Next(0, 5);
                    while (fieldLayout[idx] != 0) idx = cobber.Random.Next(0, 5);
                    fieldLayout[idx] = (byte)i;
                }

                cobber.Database.AddEntry("AntiTamper", "Key0", key0);
                cobber.Database.AddEntry("AntiTamper", "Key1", key1);
                cobber.Database.AddEntry("AntiTamper", "Key2", key2);
                cobber.Database.AddEntry("AntiTamper", "Key3", key3);
                cobber.Database.AddEntry("AntiTamper", "Key4", key4);
                cobber.Database.AddEntry("AntiTamper", "SectName", sectName);
                cobber.Database.AddEntry("AntiTamper", "FieldLayout", fieldLayout);
            ...
            public void Phase1(ModuleDefinition mod)
            {
                ...
                mutator.Mutate(cobber.Random, root);

                root.Name = cobber.Namer.GetRandomName();
                root.Namespace = "";
                AddHelper(mod, root, HelperAttribute.NoInjection);
                foreach (MethodDefinition mtdDef in root.Methods)
                {
                    if (mtdDef.IsConstructor) continue;
                    mtdDef.Name = cobber.Namer.GetRandomName();
                    AddHelper(root, mtdDef, HelperAttribute.NoInjection);
                }
                foreach (FieldDefinition fldDef in root.Fields)
                {
                    fldDef.Name = cobber.Namer.GetRandomName();
                    AddHelper(root, fldDef, HelperAttribute.NoInjection);
                }
                foreach (TypeDefinition nested in root.NestedTypes)
                {
                    ...
                    nested.Name = cobber.Namer.GetRandomName();
                    AddHelper(root, nested, HelperAttribute.NoInjection);
                    foreach (MethodDefinition mtdDef in nested.Methods)
                    {
                        ...
                        mtdDef.Name = cobber.Namer.GetRandomName();
                        AddHelper(nested, mtdDef, HelperAttribute.NoInjection);
                    }
                    foreach (FieldDefinition fldDef in nested.Fields)
                    {
                        if (fldDef.IsRuntimeSpecialName) continue;
                        fldDef.Name = cobber.Namer.GetRandomName();
                        AddHelper(nested, fldDef, HelperAttribute.NoInjection);
                    }
                }

                cobber.Database.AddEntry("AntiTamper", "Helper", root.Name);
            }
            ...
            MethodData Transform(MethodBody body, int idx, byte[] codes, Range range)
            {
                ...
                if ((codes[range.Start] & 0x3) == 0x2)
                {
                    ...
                    ret.LocalVars = 0;
                    ret.MaxStack = (uint)8;
                    ret.Options = (uint)cobber.Random.Next(0, 2) << 8;
                }
                else
                {
            ...
            public void Phase3(MetadataProcessor.MetadataAccessor accessor)
            {
                ...
                for (int i = 0; i < o.Count; i++) randArray[i] = cobber.Random.Next();
                ...
                foreach (var i in objs)
                {
                    if (i is MethodData)
                    {
                        ...
                        cobber.Database.AddEntry("AntiTamper", (0x06000001 + mtdDat.Index).ToString("X"), mtdDat.BufferOffset);
                    }
                    else
                    {
                        ...
                        cobber.Database.AddEntry("AntiTamper", strDat.String, strDat.BufferOffset);
                    }
                }
                ...
                foreach (var i in mtdDats)
                {
                    ...
                    cobber.Random.NextBytes(randBuff);
            ...
            static byte[] Encrypt(NameHelper helper, byte[] buff, byte[] dat, out byte[] iv, byte key)
            {
                dat = (byte[])dat.Clone();
            ...
            public void Phase5(Stream stream, MetadataProcessor.ImageAccessor accessor)
            {
                ...
                byte[] iv;
                byte[] dat = Encrypt(cobber.Namer, buff, ms.ToArray(), out iv, key6);
