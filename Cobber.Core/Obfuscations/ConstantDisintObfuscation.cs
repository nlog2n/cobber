...
{
    public class ConstantDisintObfuscation : StructurePhase, IObfuscation, IProgressProvider
    {
        public string Name
        {
            get { return "Constant Disintegration"; }
        }
        public string Description
        {
            get
            {
                return @"disintegrate the constants in the code into expressions.
***This obfuscation could affect the performance if your application uses constant frequently***";
            }
        }
        public string ID
        {
            get { return "const disintegrate"; }
        }
        ...
        public override void Process(object target, NameValueCollection parameters)
        {
            ...
            IList<IAnnotationProvider> targets = target as IList<IAnnotationProvider>;
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                int lv = 5;
                if (Array.IndexOf(parameters.AllKeys, mtd.GetHashCode().ToString("X8") + "_level") != -1)
                {
                    if (!int.TryParse(parameters[mtd.GetHashCode().ToString("X8") + "_level"], out lv) && (lv <= 0 || lv > 10))
                    {
