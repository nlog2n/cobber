...
{
    public class ControlFlowObfuscation : StructurePhase, IObfuscation
    {
        enum LevelType
        {
            //00: None      None
        ...
        class Statement
        {
            public int BeginStack; // fixed where first statement begins with non-zero stack
            public StatementType Type;
            public Instruction[] Instructions;
            public int Key;
        }

        public string Name { get { return "Control Flow Obfuscation"; } }
        public string Description { get { return "obfuscate the code in the methods so that decompilers cannot decompile the methods."; } }
        public string ID { get { return "control flow"; } }
        ...
        public Behaviour Behaviour { get { return Behaviour.AlterCode; } }
        public override Priority Priority { get { return Priority.MethodLevel; } }
        ...
        public Dictionary<string, List<string>> Parameters
        {
            get
            {
                ...
                {
                    {"level", new List<string> {"5", "0", "1", "2", "3", "4", "6", "7", "8", "9"}},
                    {"genjunk", new List<string> {"false","true"}},
                    {"fakeBranch", new List<string> {"false","true"}}
                };
            }
        }
        ...
        {
            method = target as MethodDefinition;
            if (!method.HasBody || method.Body.Instructions.Count == 0) return;

            level = 5;
            if (Array.IndexOf(parameters.AllKeys, "level") != -1)
            {
                if (!int.TryParse(parameters["level"], out level) && (level <= 0 || level > 10))
                {
            ...
            if (method.Module.Architecture != TargetArchitecture.I386)
                genJunk = false;
            else if (Array.IndexOf(parameters.AllKeys, "genjunk") != -1)
            {
                if (!bool.TryParse(parameters["genjunk"], out genJunk))
                {
            ...
            fakeBranch = false;
            if (Array.IndexOf(parameters.AllKeys, "fakeBranch") != -1)
            {
                if (!bool.TryParse(parameters["fakeBranch"], out fakeBranch))
                {
        ...
        IEnumerable<Instruction> GetJunk_(VariableDefinition stateVar)
        {
            ...
            else if (type.Fields.Count > 0)
            {
                ...
                if ( fields.Length > 0 ) // check
                    ...
                    yield return Instruction.Create(OpCodes.Ldtoken, type); // added
            }
            else
