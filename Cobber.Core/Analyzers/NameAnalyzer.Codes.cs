	...
	partial class NameAnalyzer
    {
        void AnalyzeCodes(MethodDefinition mtd)
        {
            for (int i = 0; i < mtd.Body.Instructions.Count; i++)
            {
                ...
                {
                    Cobber.Database.AddEntry(DB_SRC, inst.Operand.ToString(), string.Format("ldtoken @ {0} => Not renamed", mtd));
                    ...
                    inst.Operand is FieldReference)
                {
                    ...
                    else if (inst.Operand is MethodReference)
                    {
                        ...
                        if (NameDatabase.Reflections.ContainsKey(id))
                        {
                            ReflectionMethod Rmtd = NameDatabase.Reflections[id];
                            Instruction memInst;
