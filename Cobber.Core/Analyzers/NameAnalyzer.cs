...
namespace Cobber.Core.Analyzers
{
    partial class NameAnalyzer : Analyzer
    {
        ...
        internal Cobber Cr { get { return Cobber; } }
        ...
        {
            (mod as IAnnotationProvider).Annotations[RenMode] = NameMode.Letters; // .Unreadable; modified by fanghui
            //int p = 1;
            foreach (TypeDefinition type in mod.Types)
            {
                Init(type);
        ...
        void AnalyzeModule(ModuleDefinition mod)
        {
        ...
        void AnalyzeType(TypeDefinition type)
        {
            if (type.Name == "<Module>" || IsTypePublic(type))
            {
                (type as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, type.FullName, "Pub/Global type => Not renamed");
            }

            if (type.IsImport)
            {
                (type as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, type.FullName, "ComImport type => Not renamed");
            }
        ...
        void AnalyzeMethod(MethodDefinition mtd)
        {
            ...
            {
                (mtd as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, mtd.FullName, "Pub method/ctor => Not renamed");
            }
            ...
            {
                (mtd as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, mtd.FullName, "ComImport w/o DispId => Not renamed");
            }
            else if (mtd.DeclaringType.BaseType != null && mtd.DeclaringType.BaseType.Resolve() != null)
            {
                ...
                {
                    (mtd as IAnnotationProvider).Annotations[RenOk] = false;
                    Cobber.Database.AddEntry(DB_SRC, mtd.FullName, "Delegate => Not renamed");
                }
            }
        ...
        void AnalyzeField(FieldDefinition fld)
        {
            ...
            {
                (fld as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, fld.FullName, "Enum/pub field => Not renamed");
            }
        }

        void AnalyzeProperty(PropertyDefinition prop)
        {
            AnalyzeCustomAttributes(prop);
            if (prop.IsRuntimeSpecialName || IsTypePublic(prop.DeclaringType))
            {
                (prop as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, prop.FullName, "Pub prop => Not renamed");
            }
            ...
            while (baseType != null)
            {
                ...
                if (def != null)
                {
                    foreach (var i in def.Interfaces)
                        ...
                        {
                            (prop as IAnnotationProvider).Annotations[RenOk] = false;
                            Cobber.Database.AddEntry(DB_SRC, prop.FullName, "INotifyPropertyChanged => Not renamed");
                            return;
                        }
            ...
            // exclude renaming of anonymous type property 
            if (prop.DeclaringType.Name.Contains("AnonymousType"))
            {
                (prop as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, prop.FullName, "Anon type prop => Not renamed");
            }
        }

        void AnalyzeEvent(EventDefinition evt)
        {
            AnalyzeCustomAttributes(evt);
            if (evt.IsRuntimeSpecialName || IsTypePublic(evt.DeclaringType))
            {
                (evt as IAnnotationProvider).Annotations[RenOk] = false;
                Cobber.Database.AddEntry(DB_SRC, evt.FullName, "Pub evt => Not renamed");
            }
        }
        ...
        bool AnalyzeCustomAttributes(ICustomAttributeProvider ca)
        {
            ...
            foreach (var i in ca.CustomAttributes)
            {
                ...
                if (NameDatabase.ExcludeAttributes.Contains(i.AttributeType.FullName) && ca is IAnnotationProvider)
                {
                    (ca as IAnnotationProvider).Annotations[RenOk] = false;
                    Cobber.Database.AddEntry(DB_SRC, ca.ToString(), i.AttributeType.FullName + " => Not renamed");
                    ret = true;
                }
            }
            return ret;
