        ...
        void AnalyzeIvt(AssemblyDefinition asm)
        {
            ...
            foreach (ModuleDefinition mod in asm.Modules)
            {
                foreach (TypeReference typeRef in mod.GetTypeReferences())
                {
                    ...
                    {
                        Cobber.Database.AddEntry(DB_SRC, typeDef.FullName, string.Format("IVT ref @ {0} => Not renamed", asm.FullName));
                ...
                foreach (MemberReference memRef in mod.GetMemberReferences())
                {
                    ...
                    {
                        Cobber.Database.AddEntry(DB_SRC, memDef.FullName, string.Format("IVT ref @ {0} => Not renamed", asm.FullName));
