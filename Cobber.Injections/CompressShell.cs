...
// injected by class name "CompressShell" in Compressor.cs
static class CompressShell
{
    ...
    // decrypt other resources to get asm
    static Assembly DecryptAsm(object sender, ResolveEventArgs e)
    {
        ...
        try
        {
            string resName = GetNewResourceName(e.Name, Mutation.Key2I);

            Stream str = typeof(CompressShell).Assembly.GetManifestResourceStream(resName);
            if (str != null)
            {
                logFile.WriteLine("get resource " + e.Name + " OK.");
                ...
                logFile.WriteLine("decrypt resource " + e.Name + " OK.");

                asm = Assembly.Load(asmDat);
                logFile.WriteLine("load " + e.Name + " OK.");
        ...
        {
            logFile.WriteLine(e.Name + " failed to decrypt: " + ex.Message);
        }
    ...
    static StreamWriter logFile = new StreamWriter("cobberlog.txt");
    ...
    static int Main(string[] args)
    {
        try
        {
            logFile.WriteLine("packer launched.");

            Assembly asm = Assembly.GetEntryAssembly();
            Stream stream = asm.GetManifestResourceStream(Res); // get the resource stream for main module
            logFile.WriteLine("extract resource stream for main module OK.");
            byte[] asmDat;
            using (BinaryReader rdr = new BinaryReader(stream))
            {
                asmDat = rdr.ReadBytes((int)stream.Length);
            }
            asmDat = Decrypt(asmDat);
            logFile.WriteLine("decrypt resource OK.");

            // do not call any asm or module methods before loaded.
            Mod = asm.LoadModule("___.netmodule", asmDat);
            // input module name (string) must correspond to a file entry in this assembly's manifest. 
            // otherwise ArgumentException:	Specified file name was invalid.
            logFile.WriteLine("load main module OK.");
        ...
        {
            logFile.WriteLine("load main module from resource " + Res + " failed: " + ex.Message);
        }

        AppDomain.CurrentDomain.AssemblyResolve += DecryptAsm; // for resolving other asm resource on the fly

        object ret = null;
        try
        {
            // 0x06000000 = Mono.Cecil.TokenType.Method
            MethodBase m = Mod.ResolveMethod(0x06000000 | (int)modPow(Rid, 0x47, (ulong)Mutation.Key1L));
            logFile.WriteLine("resolve method " + m.Name + " OK");
            ...
            logFile.WriteLine("invoke method OK");
        }
        catch (Exception ex)
        {
            logFile.WriteLine("invoke method in main module failed: " + ex.Message + "\n" + ex.StackTrace);
        }
